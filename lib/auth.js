...
Ldap.createLdapUser = function(core, options, ldapEntry, callback) {
    ...
    var email = ldapEntry[field_mappings.email]
        ? ldapEntry[field_mappings.email].toString().split(',')[0]
        : undefined 
        ;
    ...
    if (!data.email) {
        data.email = data.uid + '@' + options.extra_options.mailbase;
    }
    if (!data.firstName) {
        data.firstName = data.displayName.toString().split(' ')[0];
    }
    if (!data.lastName) {
        data.lastName = data.displayName.toString().split(' ')[1];
    }
    if (!data.displayName) {
